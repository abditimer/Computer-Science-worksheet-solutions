...
public class Tree {
	...
	/**
	 * Tree class defines a recursive type called Tree, and provides constructor
	...
	/**
	 * Creates a multi-line String that represents this Tree. The format looks
	 * like this <code>
	 10
	 |
	 |- 14
	 ...
	 |
	 |- 6
	 </code> Where the bottom child is the left sub tree and the top child is
	 * the right sub tree. If both children are nil or the empty tree then they
	 * will not be printed. If only one child is nil then both children are
	 * printed to so it can be known which was the right child and which was the
	 * left child.
	 * 
	 * @param tree
	 *            The tree, which may not be null
	...
	/**
	 * source (with modifications) http://www.connorgarvey.com/blog/?p=82 Print
	 * a formatted representation of the given tree. The format looks like this
	 * <code>
	 10
	 |
	 |- 14
	...
	private static String formatTreeDrawLines(java.util.List<Iterator<Tree>> parentIterators, boolean amLast) {
	...
	private static StringBuilder formatTreeHelper(Tree t, StringBuilder buffer,
